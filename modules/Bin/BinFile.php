...
class Bin extends Core\Module
{
	...
	public function upload($key, &$files = null)
	{
		if (count($_FILES) < 1)
		{
			$this->logError('No file given for upload.');
			return false;
		}
		...
		if (!is_dir($dir))
		{
			$this->logError('File parent directory does not exist for uloaded file: ' . $dir);
			return false;
		}
		...
		foreach ($_FILES as $filedl)
		{
			...
			if ($filedl['error'])
			{
				$this->logError('File upload failed: ' . $filedl['name']);
				$error = true;
			}
			else
			{
				...
				{
					$this->logError('Moving uploaded file failed, destination: ' . $filename);
					$error = true;
				}
			}
			$files[] = array(
	...
	public function delete($key)
	{
		...
		if (!file_exists($path))
		{
			$this->logError('Trying to delete file that does not exist, key: ' . $key);
			return false;
		}
		...
		if (!$r)
		{
			$this->logError('Failed to delete file, key: ' . $key);
			return false;
		}
	...
	public function export($key, $format = 'zip', $echo = false, $set_headers = false)
	{
		if ($format != 'zip')
		{
			$this->logError('Invalid export format: ' . $format);
			return false;
		}
	...
	public function checkAccess($key, $throw_exception = false)
	{
		...
		foreach ($parts as $part)
		{
			...
			$accesses = @yaml_parse_file($access_file);
